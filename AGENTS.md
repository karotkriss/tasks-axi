...
- **Issue linkage + close-on-done.** `add`/`update --issue <url>` stores a `kind=issue` link (an `/issues/<n>` URL living in the title prose, mirroring `--pr`).
  A fresh `done` then closes every linked issue via the local `gh` CLI (`--reason completed`, or `not planned` for a `--dropped` close), commenting with the recorded PR URL when one exists - fail-soft: the done-transition always lands first, and a failed close becomes an `issue: CLOSE FAILED` notice plus a manual-close hint (`issue_close` outcomes under `--json`).
  This lives in the CLI layer (`src/gh.ts` owns the gh invocation; `context.issueCloser` is the test seam), never a Store backend, so every backend inherits it.
